...
public class LearningFacadeUnitTest {
    ...
    class FindChapterWithProgressByUserId{
        ...
        ChapterProgressDetailResponse chapterProgress1Details;
        ChapterProgressDetailResponse chapterProgress2Details;
        ChapterProgressDetailResponse chapterProgress3Details;
        ...
        void setUp(){
            ...
            chapterProgress1Details = ChapterProgressDetailResponse.create(chapter1Id, chapter1.getName(), chapter1.getDescription(), chapterProgress1.getTotalUnits(), chapterProgress1.getCompletedUnits());
            chapterProgress2Details = ChapterProgressDetailResponse.create(chapter2Id, chapter2.getName(), chapter2.getDescription(), chapterProgress2.getTotalUnits(), chapterProgress2.getCompletedUnits());
            chapterProgress3Details = ChapterProgressDetailResponse.create(chapter3Id, chapter3.getName(), chapter3.getDescription(), chapterProgress3.getTotalUnits(), chapterProgress3.getCompletedUnits());
        }
        ...
        void 전체_챕터와_진행도_조회에_성공한다(){
            ...
            // when
            learningFacade.getAllChapters(validUserId);
    ...
    class SaveLearningResult{
        ...
        LearningResultSaveRequest request, invalidRequest;

        ChapterProgress chapterProgress, invalidUserChapterProgress;
        UnitProgress unitProgress, invalidUserUnitProgress;
        LessonProgress lessonProgressForFirstAttempt, lessonProgressForRetry, invalidUserLessonProgress;
        ...
        void setUp(){
            List<ProblemResultRequest> problemResultRequests = List.of(
                    new ProblemResultRequest(problem1Id, true, 0L),
                    new ProblemResultRequest(problem2Id, true, 0L)
            );

            request = new LearningResultSaveRequest(lessonId, 180, 100, problemResultRequests);
            invalidRequest = new LearningResultSaveRequest(invalidLessonId, 180, 100, problemResultRequests);
        ...
        void 첫번째_문제_풀이_결과_저장에_성공한다(){
            // given
            when(lessonProgressService.getLessonProgressAndUpdateStatus(request.lessonId(), userId, request.learningTime()))
            ...
            // then
            verify(lessonProgressService).getLessonProgressAndUpdateStatus(request.lessonId(), userId, request.learningTime());
        ...
        void 레슨_조회에_실패하면_예외를_반환한다(){
            // given
            when(lessonProgressService.getLessonProgressAndUpdateStatus(invalidLessonId, userId, 180))
                    .thenThrow(new RestApiException(CustomErrorCode.LESSON_NOT_FOUND));
        ...
        void 레슨이_속한_챕터와_유닛_조회에_실패하면_예외를_반환한다(){
            // given
            when(lessonProgressService.getLessonProgressAndUpdateStatus(lessonId, userId, 180))
                    .thenReturn(lessonProgressForFirstAttempt);
        ...
        void 유저_조회에_실패하면_예외를_반환한다(){
            // given
            when(lessonProgressService.getLessonProgressAndUpdateStatus(lessonId, invalidUserId, 180))
