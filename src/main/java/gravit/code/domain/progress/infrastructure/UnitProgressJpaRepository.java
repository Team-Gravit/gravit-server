...
public interface UnitProgressJpaRepository extends JpaRepository<UnitProgress,Long> {
    ...
    @Query("""
        SELECT new gravit.code.domain.progress.dto.response.UnitProgressDetailResponse(u.id, u.name, u.totalLessons, COALESCE(CAST(up.completedLessons as long), 0L))
        FROM Unit u
