...
public interface ChapterProgressJpaRepository extends JpaRepository<ChapterProgress, Long> {
    ...
    @Query("""
        SELECT new gravit.code.domain.progress.dto.response.ChapterProgressDetailResponse(c.id, c.name, c.description, c.totalUnits, COALESCE(CAST(cp.completedUnits as long), 0L))
        FROM Chapter c
    ...
    @Query("""
        SELECT new gravit.code.domain.progress.dto.response.ChapterSummaryResponse(c.id, c.name, c.totalUnits, cp.completedUnits)
