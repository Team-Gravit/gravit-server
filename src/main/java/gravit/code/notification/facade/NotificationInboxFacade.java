...
public class NotificationInboxFacade {
    ...
    public List<NotificationResponse> getInbox(long userId) {
        ...
        Set<Long> alreadyFollowing = followTargetIds.isEmpty()
                ...
                : friendService.findFollowingIdsAmong(userId, followTargetIds);

        Map<Long, UserSummaryDto> userIdToActor = userService.getUserSummaries(followTargetIds);
        ...
        return notifications.stream()
                .map(n -> toResponse(n, alreadyFollowing, userIdToActor, congratulatedFeedIds))
                .toList();
    }
    ...
    private NotificationResponse toResponse(
            ...
            Map<Long, UserSummaryDto> userIdToActor,
        ...
        NotificationActor actor = isFollow
                ? toActor(userIdToActor.get(notification.getTargetId()))
                : null;
    ...
    private NotificationActor toActor(UserSummaryDto summary) {
        if (summary == null) {
            return null;
        }
        return NotificationActor.of(summary.id(), summary.nickname(), summary.profileImgNumber());
    }
}
