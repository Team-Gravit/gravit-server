...
public class NotificationMessageProvider {
    ...
    private static final List<SeasonEndingMilestoneDto> SEASON_ENDING_MILESTONES = List.of(
            new SeasonEndingMilestoneDto(7, "시즌이 일주일 뒤 끝나요!", "지금이 티어 올릴 마지막 기회예요 💪"),
            new SeasonEndingMilestoneDto(3, "시즌 종료가 3일 앞으로 다가왔어요!", "마지막까지 달려봐요 🔥")
    );
    ...
    private static final List<InactivityMilestoneDto> INACTIVITY_MILESTONES = List.of(
            new InactivityMilestoneDto(7, "벌써 일주일이 지났어요 😢 Gravit이 기다리고 있어요!"),
            new InactivityMilestoneDto(14, "14일이 지났어요. 슬슬 돌아올 때가 된 것 같은데요? 👀"),
            new InactivityMilestoneDto(30, "한 달 동안 보고 싶었어요 😭 지금 돌아와도 늦지 않아요!")
    );
    ...
    public List<InactivityMilestoneDto> inactivityMilestones() {
    ...
    public String inactivity(int inactiveDays) {
        return INACTIVITY_MILESTONES.stream()
                ...
                .map(InactivityMilestoneDto::message)
                .findFirst()
    ...
    public List<SeasonEndingMilestoneDto> seasonEndingMilestones() {
