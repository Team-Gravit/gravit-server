...
VALUES (285, 993, '스칼라 서브쿼리, scalar subquery, 스칼라', 'SELECT 절에 사용되는 서브쿼리를 스칼라 서브쿼리라고 한다. 스칼라 서브쿼리는 정확히 하나의 값(1행 1컬럼)을 반환해야 한다.'),
       (286, 996, '인라인 뷰, inline view, 인라인', 'FROM 절에 사용되는 서브쿼리를 인라인 뷰라고 한다. 인라인 뷰는 임시 테이블처럼 동작하며, 메인 쿼리에서 일반 테이블처럼 사용할 수 있다.');
