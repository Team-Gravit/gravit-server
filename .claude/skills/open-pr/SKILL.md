...
4. 문체
   ...
   - 용어는 현업에서 통용되는 쪽을 골라라. (`전체 스캔` 대신 `풀스캔`, `검사 행` 대신 `스캔 행`, `플래너` 대신 `옵티마이저`, `술어` 대신 `WHERE 조건`)
5. 블록 구분
   - `<br>`은 섹션(`##`) 사이뿐 아니라 **굵은 소제목 블록 사이마다** 넣어라.
     문제 1과 문제 2 사이, 해결 2와 해결 3 사이, 표와 다음 소제목 사이가 모두 해당한다.
   - 표에 이름을 붙일 때는 굵게 처리하고 그 앞뒤에도 `<br>`을 넣어라. (예: `**챕터별 집계**`)
...
2. Before와 After가 있는 수치는 표로 제시하고 **변화** 칼럼을 붙인다.
   ...
   - 소제목 옆에 측정 기준을 괄호로 밝힌다. (예: `**문제 2 - 문제 제출 풀스캔** (200만 행, 상위 7건 조회 기준)`)
3. 쿼리 개선이면 표를 두 종류로 나눈다. 한 표에 섞지 마라.
   - **실행 계획 표**: 실행 계획, 스캔 행, 필터로 버린 행, buffers, 실행시간 (EXPLAIN ANALYZE)
...
5. 인덱스를 추가했으면 인덱스 정의를 한 줄로 먼저 적고, **컬럼마다 그 자리에 둔 이유를 문단으로** 밝힌다.
   ...
   등호 조건과 범위 조건의 순서, 커버링(INCLUDE)으로 뺀 이유를 반드시 포함한다.
