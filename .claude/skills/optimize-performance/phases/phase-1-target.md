...
3. 대상 엔드포인트의 **슬러그**를 정한다. 이 슬러그가 대상 디렉토리 이름이 된다.
   - 경로의 마지막 세그먼트를 케밥 케이스로 쓴다. (`/api/v1/my-pages/learning/top-chapters` → `top-chapters`)
   - 마지막 세그먼트가 경로 변수(`{id}`)면 그 앞 세그먼트를 쓴다.
   - 같은 이슈의 기존 대상과 겹치면 앞 세그먼트를 하나 더 붙인다. (`learning-top-chapters`)
   - 정한 슬러그를 호출자에게 알린다.

4. 실행 경로를 Controller → Facade → Service → Repository 순으로 읽고, **호출자에게 예상 쿼리를 먼저 묻는다.**
   ...
   - 답을 받으면 실제 코드와 대조해 **누락분과 오차만** 짚는다. 특히 아래를 확인한다.
     - 호출자가 놓친 지연 로딩 지점
     - 인증 필터, 인터셉터처럼 대상 API 밖에서 붙는 쿼리
     - 메서드 하나가 실제로는 여러 쿼리로 쪼개지는 경우
   - 호출자가 답하기 어려워하면 그때 목록을 제시하고 근거가 되는 코드 위치를 함께 보여준다.

   사전 관찰로 인덱스 유무를 적을 때 **마이그레이션 파일을 근거로 쓰지 마라.**
   `CREATE INDEX`가 마이그레이션에 있다는 사실은 그 인덱스가 DB에 있다는 근거가 못 된다.
   Phase 2의 6)에서 실측으로 확인하므로, 여기서는 `미확인`으로 두거나 확인 시점을 함께 적는다.
...
5. 작업 디렉토리 `.claude/resources/perf/{이슈번호}/{슬러그}/`를 만들고 그 안에 `record.md`를 생성한다.
   **이번 대상 것 하나만 만든다.**
   ...
   - 셸 변수를 잡도록 호출자에게 제시한다. 이후 모든 셸 명령이 이 두 변수를 쓴다.
