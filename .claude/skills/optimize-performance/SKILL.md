...
| 자원 | 동시에 진행하면 생기는 일 |
|---|---|
| `pg_stat_statements` (인스턴스 전역) | 리셋 없이 다음 대상을 재면 통계가 섞인다. `per_req`의 분모가 그 대상의 `requests`이므로 요청당 쿼리 수가 틀린 값이 된다 |
...
- 비교 대상이 아닌 일회성 조회(Phase 3-A의 행 수 확인, Phase 5-B의 `\d`와 `pg_stats`)는 파일로 남기지 않는다.
...
- Phase 3-A: 같은 이슈의 다른 대상에서 이미 데이터를 채웠으면 → **3-B** (건너뜀)
