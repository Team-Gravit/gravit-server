...
- Redis (리프레시 토큰, 메일 인증 코드, 리그 랭킹, 이벤트 재시도 큐, 시즌 캐시)
- Spring Security + OAuth2 (Google, Kakao, Naver) + JWT
- Spring AI OpenAI 스타터 + LiteLLM 게이트웨이 (면접 답변 채점)
- AWS S3 SDK (면접 음성 원본 저장)
- Firebase Admin (FCM 푸시) / Spring Mail / Spring Retry
- springdoc-openapi (Swagger UI)
- Testcontainers (통합 테스트) / H2 (빌드 시 `flywayValidate` 전용)
- Actuator + Micrometer (메트릭 노출) / Prometheus, Grafana (앱센터 중앙 모니터링 레포) / Loki + Alloy (dev, prod compose 로그 수집)
...
- 테스트 작성 규칙 (`src/test/java/**`) → `test-convention.md`
- API 문서 작성 규칙 (Docs 인터페이스) → `api-docs-convention.md`
- 코드 컨벤션 → `code-convention/`
  - 공통 (네이밍, 포맷팅, 예외, 객체 생성, 상수, 주석, 레이어 흐름) → `common.md`
  - Entity(domain) + DB 매핑 → `domain.md`
  - DTO(Request, Response, Internal, Event) → `dto.md`
...
`.claude/resources/` - 작업 산출물 (자동 로드 아님)

- 계획서 → `plans/PLAN-{이슈번호}.md`
- 성능 측정 기록 → `perf/{이슈번호}/`
