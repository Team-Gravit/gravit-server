...
- `.github/PULL_REQUEST_TEMPLATE.md`: 섹션 제목을 `###`에서 `##`로 올리고 섹션 사이에 `<br>`을 넣음 - 이유: 계획에서는 장식을 모두 걷어내기로 했으나 실제 렌더링에서 섹션 구분이 약했다. 구분선(`---`)까지 넣어봤다가 과하다고 판단해 최종적으로 제목 크기와 `<br>`만 남겼다. open-pr 스킬에는 이 장식을 지우지 말라는 지시를 추가했다.
