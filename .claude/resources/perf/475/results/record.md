...
| 요청 수 | 3610 |
| 요청당 쿼리 수 | 약 66건 (상위 20건 per_req 합계 66.21. 나머지는 전체 시간의 1.3%) |
...
- 근거:
  ...
  - 요청당 DB 실행 시간 79.96 ms 대 duration med 1005.87 ms.
    DB 실행이 응답 시간의 8%다. 쿼리 약 66건의 왕복 오버헤드와,
...
| 요청 수 | 9876 |
| 요청당 쿼리 수 | 약 47건 (상위 20건 기준. 오답노트 구간이 1로 접힌 뒤) |
| **대상 요청당 호출 수** | **30.00** |
...
| | 실패율 / check | 0 / 1 | 0 / 1 | 동일 |
| 하드웨어 독립 | 요청당 쿼리 수 | 약 66건 | 약 18건 | −72.3% |
