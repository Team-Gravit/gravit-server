// PERF-494 부하 테스트 스크립트: GET /api/v1/lessons/{unitId}
//
...
export default function () {
    ...
    const res = http.get(`${BASE_URL}/api/v1/lessons/${TARGET_UNIT_ID}`, params);

    check(res, {
        'status is 200': (r) => r.status === 200,
        'body is not empty': (r) => r.body !== null && r.body.length > 2,
        'lessonSummaries 2건이 실려 있다': (r) => {
            const body = parseBody(r);
            return body !== null
                && body.lessonSummaries !== undefined
                && body.lessonSummaries.length === EXPECTED_LESSON_COUNT;
        },
    });
}
